...
OUTPUT_PATH = os.path.join(os.getcwd(), "evaluation", "all_responses_grading_separated_vllm_hercule_with_reference.csv")
...
def robust_absolute_grade(judge, instruction, response, rubric, reference_answer, max_attempts=3, sleep_time=2):
...
for idx, row in df.iterrows():
    ...
    for rubric_idx, rubric in enumerate(score_rubrics, start=1):
        ...
        total_tokens = count_tokens_hf(instruction, response, rubric_text, reference_answer)
        if total_tokens > MAX_TOKENS:
            ...
            feedback, score = robust_absolute_grade(
                judge, instruction, response, rubric_text, reference_answer
            )
