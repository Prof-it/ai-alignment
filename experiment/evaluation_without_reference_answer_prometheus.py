...
for rubric_idx, rubric in enumerate(score_rubrics, start=1):
    ...
    for idx, row in df.iterrows():
        ...
        reference_answer = ""  # Provide reference answers if available
    ...
    # Batch grade the filtered inputs
    try:
...
grading_df.to_csv(
    os.path.join(os.getcwd(), "evaluation", "all_responses_grading_separated_vllm.csv"),
    index=False
)